...
double cotEdge(DefaultTriMesh& mesh, DefaultTriMesh::EdgeHandle eh) {
    ...
    double sum = 0.0;
    if (!mesh.is_boundary(he0)) sum += cotHalfedge(mesh, he0);
    if (!mesh.is_boundary(he1)) sum += cotHalfedge(mesh, he1);
    return sum;
}
...
bool computeConjugateSurface(DefaultTriMesh& mesh,
    ...
    for (auto fh = mesh.fh_begin(f0); fh != mesh.fh_end(f0); ++fh) {
        ...
        if (!mesh.is_boundary(he) &&
            !mesh.is_boundary(mesh.opposite_halfedge_handle(he))) {
            he0 = he; break;
        }
    }
    if (!he0.is_valid()) {
    ...
    while (!front.empty()) {
        H fr = front.front(); front.pop();
        if (mesh.is_boundary(fr)) continue;
        H fropp = mesh.opposite_halfedge_handle(fr);
        if (mesh.is_boundary(fropp)) continue;
    ...
    // ── Validation: cotangent Laplacian ──────────────────────
    {
        ...
        for (auto vh = conj.vertices_begin(); vh != conj.vertices_end(); ++vh) {
            ...
            if (L > maxLap) maxLap = L;
            sumLap += L;
        }
        double avgLap = (nInterior > 0) ? sumLap / nInterior : 0.0;
        std::cout << "validation: |cot-Laplacian| max=" << maxLap
                  << " avg=" << avgLap
    ...
    std::cout << "Conjugate surface saved: " << outFile
              << " (nv=" << conj.n_vertices()
              << " nf=" << conj.n_faces() << ")\n";
    return true;
}
