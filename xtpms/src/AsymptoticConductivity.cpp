...
Compile1ring::Compile1ring(const Eigen::Vector3d& o, const std::vector<Eigen::Vector3d>& ring) {
	...
	for (int i = 0; i < N; ++i) {
		...
		double denom_A = 2.0 * std::sqrt(e_sq0 * e_sq2);
		double denom_B = 2.0 * std::sqrt(e_sq1 * e_sq2);
		if (denom_A < 1e-30 || denom_B < 1e-30) continue;

		double cosA = std::clamp((e_sq0 + e_sq2 - e_sq1) / denom_A, -1.0, 1.0);
		double cosB = std::clamp((e_sq1 + e_sq2 - e_sq0) / denom_B, -1.0, 1.0);
		double sinA = std::sqrt(1.0 - cosA * cosA);
		double sinB = std::sqrt(1.0 - cosB * cosB);
		double cotA = (sinA > 1e-15) ? cosA / sinA : 0.0;
		double cotB = (sinB > 1e-15) ? cosB / sinB : 0.0;

		double denom_theta = 2.0 * std::sqrt(e_sq0 * e_sq1);
		double cos_theta = (denom_theta > 1e-30) ?
			std::clamp((e_sq0 + e_sq1 - e_sq2) / denom_theta, -1.0, 1.0) : 0.0;
		double theta_i = std::acos(cos_theta);
		theta_sum += theta_i;
		...
		// 角度加权法向
		if (axb_norm > 1e-15) nv += (axb / axb_norm) * theta_i;
...
Eigen::Matrix<double, 2, 3> scalarGradientMatrix(
	const Eigen::Matrix3d& tri, const Eigen::Matrix3d& fr) {
	Eigen::Matrix2d V;
	V.col(0) = fr.leftCols<2>().transpose() * (tri.col(1) - tri.col(0));
	V.col(1) = fr.leftCols<2>().transpose() * (tri.col(2) - tri.col(0));
...
void tailorADC(PeriodicTriMesh& mesh, const TailorADCOptions& opts) {
	...
	// 固定 remesh 目标边长：在首次迭代前用平均边长计算，后续不变
	...
	if (opts.enableRemesh && remeshOpts.targetLength < 0) {
		...
		for (auto e_it = mesh.edges_begin(); e_it != mesh.edges_end(); ++e_it) {
			HH he = mesh.halfedge_handle(*e_it, 0);
			Vec3d ev = makePeriod(mesh.point(mesh.to_vertex_handle(he)) - mesh.point(mesh.from_vertex_handle(he)), hp);
			totalLen += ev.norm();
			++edgeCount;
		}
		remeshOpts.targetLength = totalLen / edgeCount;
		std::cout << "fixed remesh targetLength = " << remeshOpts.targetLength << "\n";
	}

	for (int iter = 0; iter < opts.maxIter; ++iter) {
		// [1] surgery
		if (opts.enableSurgery && iter % opts.surgeryInterval == 0 && iter > 0) {
