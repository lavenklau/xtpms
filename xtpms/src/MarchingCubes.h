...
/// 0=(0,0,0), 1=(1,0,0), 2=(1,1,0), 3=(0,1,0), 4=(0,0,1), 5=(1,0,1), 6=(1,1,1), 7=(0,1,1) in cell-local axes.
...
struct ExtractMarchingCubesOptions {
	/// If true, reuse one mesh vertex per undirected pair of nodal indices along a cut edge (consistent tessellation across adjacent cells).
	bool weldSharedEdges{true};
};

/// Classic marching cubes on a sparse set of voxels. Skips a cell if any corner index is out of range for `nodes`.
/// Corner `c` counts as "inside" when `nodes[corner[c]].value < isoValue` (negative-distance style SDF with iso 0).
