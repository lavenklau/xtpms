...
// Cotangent values explode as opposite angles approach 0 or π (sin → 0).
// Clamp |cot| to keep mean-curvature / Laplacian numerically stable; beyond
// this threshold the edge weight behaves like a capped discrete operator.
// cot(≈0.057°) ≈ 1e3; angles smaller than that are treated as degenerate.
static constexpr double kCotAbsMax = 1e3;

static double clampCotangent(double cot) {
	return std::clamp(cot, -kCotAbsMax, kCotAbsMax);
}
...
Compile1ring::Compile1ring(const Eigen::Vector3d& o, const std::vector<Eigen::Vector3d>& ring) {
	...
	for (int i = 0; i < N; ++i) {
		esq_inc[static_cast<std::size_t>(i)] =
			(ring[static_cast<std::size_t>(i)] - o).squaredNorm();
		esq_opp[static_cast<std::size_t>(i)] =
			(ring[static_cast<std::size_t>((i + 1) % N)] - ring[static_cast<std::size_t>(i)])
				.squaredNorm();
	}
	...
	for (int i = 0; i < N; ++i) {
		...
		if (dA > 1e-30 && dB > 1e-30) {
			...
			double cotARaw = (sinA > 1e-15) ? cosA / sinA : 0.0;
			double cotBRaw = (sinB > 1e-15) ? cosB / sinB : 0.0;
			// Opposite angle ~0/π → |cot|→∞; Meyer H∝|Hv|/As cancels large cot in both
			// numerator and denominator, so clamping both independently is unsafe.
			// Fall back to Uniform for the whole 1-ring instead.
			if (std::abs(cotARaw) > kCotAbsMax || std::abs(cotBRaw) > kCotAbsMax)
				degenerateCot = true;
			cot_alpha[static_cast<std::size_t>((i + 1) % N)] = clampCotangent(cotARaw);
			cot_beta[static_cast<std::size_t>(i)] = clampCotangent(cotBRaw);
		}
		double dT = 2.0 * std::sqrt(e0 * e1);
		theta_v[static_cast<std::size_t>(i)] =
			(dT > 1e-30) ? std::acos(std::clamp((e0 + e1 - e2) / dT, -1.0, 1.0)) : 0.0;
	}
	...
	for (int i = 0; i < N; ++i) {
		...
		if (axb_norm > 1e-15)
			nv += (axb / axb_norm) * theta_v[static_cast<std::size_t>(i)];

		if (degenerateCot) {
			// Uniform Laplacian: equal edge weights + barycentric area (1/3 triangle area)
			Hv += a;
			As += axb_norm / 6.0;
		} else {
			// Cotangent Laplacian (Meyer): opposite-angle weights
			// Weight of edge (o, ring[i]) = (cot_alpha[i] + cot_beta[i]) / 2
			Hv += (cot_alpha[static_cast<std::size_t>(i)] + cot_beta[static_cast<std::size_t>(i)]) /
				  2.0 * a;
...
std::vector<double> computeSingularityMeasure(const PeriodicTriMesh& mesh, int surgeryType) {
	const std::size_t nv = mesh.n_vertices();
	// Intrinsic Delaunay on a connectivity copy: vertex positions/indices unchanged,
	// but near-degenerate opposite angles are repaired before cot / 1-ring curvature.
	PeriodicTriMesh idtMesh = mesh;
	makeIntrinsicDelaunay(idtMesh);
	auto geom = computeVertexGeometry(idtMesh);
	std::vector<double> singMeasure(nv, 0.0);
	for (std::size_t i = 0; i < nv; ++i) {
		const double H = geom.vrings[i].H;
		const double K = geom.vrings[i].K;
		if (surgeryType == 1) {
			singMeasure[i] = std::abs(H);
		} else {
			const double disc = std::abs(H * H - K);
...
double computeCotWeight(const PeriodicTriMesh& mesh, EH eh) {
	...
	for (int side = 0; side < 2; ++side) {
		...
		if (std::abs(sinA) < 1e-15)
			continue;
		// Clamp: near-degenerate opposite angles make cos/sin explode
		cot_sum += clampCotangent(cosA / sinA);
	}
	return cot_sum / 2.0;
}
