...
bool nearAnyFaceOfBox(const Vec3d& p, const Vec3d& boxMin, const Vec3d& boxMax, double tol) {
	for (int i = 0; i < 3; ++i) {
		...
		if (std::abs(pi - static_cast<double>(boxMin[i])) <= tol)
			return true;
		if (std::abs(pi - static_cast<double>(boxMax[i])) <= tol)
			return true;
	}
	return false;
}
...
bool isAxisPeriodDisplacement(const Vec3d& d, int axis, double periodLen, double tol) {
	...
	for (int i = 0; i < 3; ++i) {
		...
		if (i == axis) {
			if (std::abs(std::abs(di) - periodLen) > tol)
				return false;
		} else {
			if (std::abs(di) > tol)
				return false;
...
void buildSpatialGrid(const std::vector<Vec3d>& pts,
					  double cell,
...
bool findPartnerInCells(const Vec3d& p,
						const std::vector<Vec3d>& pts,
						const std::unordered_map<Cell3, std::vector<int>, Cell3Hash>& grid,
						int selfIdx,
						const std::array<double, 3>& periodLen,
						double tol,
						double invCell) {
				...
				for (int dy = -1; dy <= 1; ++dy) {
					for (int dz = -1; dz <= 1; ++dz) {
						...
						for (int j : it->second) {
							...
							if (isAxisPeriodDisplacement(d, ax, L, tol))
								return true;
						}
					}
				}
...
xtpms::DefaultTriMesh makeGyroidMC(const Vec3d& halfPeriod, int res = 24) {
	...
	return makeTPMSMC(
		...
		[Lx, Ly, Lz](double x, double y, double z) {
			...
			return std::sin(px) * std::cos(py) + std::sin(py) * std::cos(pz) +
				   std::sin(pz) * std::cos(px);
		},
		res);
}
...
xtpms::DefaultTriMesh makeSchwarzPMC(const Vec3d& halfPeriod, int res = 24) {
	...
	return makeTPMSMC(
		...
		[Lx, Ly, Lz](double x, double y, double z) {
			...
			return std::cos(2.0 * M_PI * x / Lx) + std::cos(2.0 * M_PI * y / Ly) +
...
TEST(MergePeriodBoundary, GyroidEqualPeriod_NoBoundaryEdges) {
	...
	EXPECT_EQ(bndAfter, 0)
		<< "After mergePeriodBoundary, mesh should be closed (no boundary edges)";
	EXPECT_GT(mesh.n_vertices(), 0u);
	EXPECT_GT(mesh.n_faces(), 0u);
}
...
TEST(PeriodicMeshPeriodize, StlPeriodizeSymmetry) {
	...
	for (int i = 0; i < static_cast<int>(cand.size()); ++i) {
		if (!findPartnerInCells(
				pts[static_cast<size_t>(i)], pts, grid, i, periodLen, tol, invCell)) {
			++missing;
		}
	}

	const double ratio = 1.0 - static_cast<double>(missing) / static_cast<double>(cand.size());
...
xtpms::PeriodicTriMesh loadPeriodicMeshLikeCli(const xtpms::DefaultTriMesh& src, const Vec3d& hp) {
	xtpms::DefaultTriMesh work = src;
	Vec3d bmin, bmax;
	meshBBox(work, bmin, bmax);

	double tol = 1e-4 * std::min({static_cast<double>(bmax[0] - bmin[0]),
								  static_cast<double>(bmax[1] - bmin[1]),
								  static_cast<double>(bmax[2] - bmin[2])});
	for (auto v = work.vertices_begin(); v != work.vertices_end(); ++v) {
		auto p = work.point(*v);
		for (int i = 0; i < 3; ++i) {
	...
	for (auto v = work.vertices_begin(); v != work.vertices_end(); ++v)
		mesh.add_vertex(work.point(*v));
	for (auto f = work.faces_begin(); f != work.faces_end(); ++f) {
		auto fv = work.cfv_iter(*f);
		int a = (*fv).idx();
		++fv;
		int b = (*fv).idx();
		++fv;
...
TEST(PipelineDiag, RemeshThenMerge_StepByStep) {
	...
	std::cout << "[step0] after makeClosedGyroid: nv=" << mesh.n_vertices()
			  << " nf=" << mesh.n_faces() << " boundary_edges=" << countBoundaryEdges(mesh) << "\n";
	...
	std::cout << "[step1] after remesh: nv=" << mesh.n_vertices() << " nf=" << mesh.n_faces()
	...
	if (bndAfterRemesh > 0) {
		...
		std::cout << "[step2] after merge: nv=" << mesh.n_vertices() << " nf=" << mesh.n_faces()
...
TEST(PipelineDiag, RemeshSizeStability) {
	...
	for (int i = 0; i < 3; ++i) {
		...
		int euler = V - E + F;
		std::cout << "fixed remesh #" << i << ": nv=" << V << " nf=" << F << " euler=" << euler
				  << " bnd=" << bnd << "\n";
...
TEST(PipelineDiag, RemeshBoundaryDiagnostic) {
	...
	for (std::size_t ci = 0; ci < cases.size(); ++ci) {
		...
		int bndBefore = countBoundaryEdges(mesh);
		int eulerBefore = static_cast<int>(mesh.n_vertices()) - static_cast<int>(mesh.n_edges()) +
						  static_cast<int>(mesh.n_faces());
		...
		int bndAfter = countBoundaryEdges(mesh);
		int eulerAfter = static_cast<int>(mesh.n_vertices()) - static_cast<int>(mesh.n_edges()) +
						 static_cast<int>(mesh.n_faces());

		std::cout << "case " << ci << " hp=(" << tc.hp[0] << "," << tc.hp[1] << "," << tc.hp[2]
				  << ") res=" << tc.res << " tgtLen=" << tc.targetLen
				  << ": nv=" << mesh.n_vertices() << " nf=" << mesh.n_faces() << " bnd=" << bndAfter
				  << " euler=" << eulerAfter << "\n";
...
TEST(PipelineDiag, RemeshVisualCheck) {
	...
	for (int i = 0; i < 1; ++i) {
		...
		std::cout << "step" << (i + 1) << ": nv=" << mesh.n_vertices() << " nf=" << mesh.n_faces()
				  << " bnd=" << countBoundaryEdges(mesh) << " time=" << ms << "ms\n";
	}
	EXPECT_EQ(countBoundaryEdges(mesh), 0);
}
...
TEST(AsymptoticConductivity, SensitivityFiniteDifference) {
	...
	// 沿法向二分搜索投影到隐函数水平集
	{
		...
		for (auto v_it = mesh.vertices_begin(); v_it != mesh.vertices_end(); ++v_it) {
			int vi = (*v_it).idx();
			Eigen::Vector3d p(static_cast<double>(mesh.point(*v_it)[0]),
			...
			if (flo * fhi < 0) {
				for (int iter = 0; iter < 50; ++iter) {
					...
					double fmid =
						levelSet(p[0] + tmid * n[0], p[1] + tmid * n[1], p[2] + tmid * n[2]);
					if (fmid * flo < 0) {
						thi = tmid;
						fhi = fmid;
					} else {
				...
				mesh.set_point(*v_it,
							   Vec3d(static_cast<xtpms::DefaultTriMesh::Scalar>(p[0] + t * n[0]),
				...
				double gy = -M_PI * std::sin(M_PI * p[1]) +
							0.4 * std::sin(2 * M_PI * p[0]) * M_PI * std::cos(M_PI * p[1]);
	...
	auto saveUnitCellWithScalar = [](const xtpms::PeriodicTriMesh& m,
		...
		const double L[3] = {
			2.0 * m.halfPeriod()[0], 2.0 * m.halfPeriod()[1], 2.0 * m.halfPeriod()[2]};
		const double hp[3] = {
		...
		for (auto f = m.faces_begin(); f != m.faces_end(); ++f) {
			...
			for (int k = 0; k < 3; ++k) {
				bool moved = false;
				for (int a = 0; a < 3; ++a)
					if (std::abs(p[k][a] - verts[idx[k]][a]) > 1e-8) {
						moved = true;
						break;
					}
				if (moved) {
		...
		for (int i = 0; i < nv; ++i) {
			...
			ofs << "v " << verts[i][0] << " " << verts[i][1] << " " << verts[i][2] << " " << c[0]
				<< " " << c[1] << " " << c[2] << "\n";
		}
		...
		for (int i = 0; i < nv; ++i)
			tfs << i << " " << i << " " << verts[i][0] << " " << verts[i][1] << " " << verts[i][2]
				<< " " << scalar[i] << "\n";
		for (int i = 0; i < (int)extraVerts.size(); ++i)
			tfs << nv + i << " " << extraOrigIdx[i] << " " << extraVerts[i][0] << " "
				<< extraVerts[i][1] << " " << extraVerts[i][2] << " " << scalar[extraOrigIdx[i]]
				<< "\n";
	};
	...
	for (const auto& oc : objCases) {
		...
		for (const auto& vc : vnCases) {
			...
			for (int i = 0; i < nv; ++i) {
				...
				vn[i] = vc.fn(static_cast<double>(pt[0]),
			...
			auto perturbObj = [&](double s) {
				...
				for (auto v_it = copy.vertices_begin(); v_it != copy.vertices_end(); ++v_it) {
					int vi = (*v_it).idx();
					Eigen::Vector3d p(static_cast<double>(copy.point(*v_it)[0]),
					...
					p += s * vn[vi] * n;
					copy.set_point(*v_it,
								   Vec3d(static_cast<xtpms::DefaultTriMesh::Scalar>(p[0]),
			...
			for (double stepFD : {1e-1, 5e-2, 1e-2, 5e-3, 1e-3, 1e-4}) {
				...
				std::cout << "    step=" << stepFD << "  obj+=" << objP << "  obj-=" << objM
						  << "  meshFD=" << dk_meshFD << "\n";
			}
		}
	}
...
struct RevolutionProfile {
	...
	xtpms::PeriodicTriMesh generateClosed(const Vec3d& hp, int res, bool project = false) const {
		...
		if (project)
			projectToSurface(src, hp);
		auto mesh = makeClosedTPMS(hp, src);
		if (project)
			projectToSurface(mesh, hp);
		return mesh;
	}
};
...
struct Compile1ring {
	...
	Compile1ring(const Eigen::Vector3d& o, const std::vector<Eigen::Vector3d>& ring) {
		...
		for (int i = 0; i < N; i++) {
			...
			cot_beta[i] = cotB;
			theta_v[i] = std::acos(std::clamp(
				(e_sq[0] + e_sq[1] - e_sq[2]) / 2 / std::sqrt(e_sq[0] * e_sq[1]), -1.0, 1.0));
		}
		for (int i = 0; i < N; i++) {
			...
			if (e_sq[0] + e_sq[1] >= e_sq[2] && e_sq[1] + e_sq[2] >= e_sq[0] &&
				e_sq[2] + e_sq[0] >= e_sq[1]) {
...
Eigen::Matrix<double, 2, 3> scalar_gradient_matrix(const Eigen::Matrix3d& tri,
...
TEST(AsymptoticConductivity, RevolutionSurface_K11) {
	...
	for (const auto& tc : cases) {
		...
		for (int res : resolutions) {
			...
			std::cout << "  res=" << res << " nv=" << mesh.n_vertices() << " nf=" << mesh.n_faces()
					  << " k11_num=" << k11_num << " err=" << err << " rel=" << rel_err << "\n";
	...
	for (int res : resolutions) {
		...
		auto sensRaw = xtpms::computeSensitivity(mesh, geom, ulist);
		{
			...
			for (double step : {1e-2, 1e-3}) {
				...
				std::cout << "  Area deriv (vn=1): step=" << step << "  FD=" << dAs_fd
						  << "  analytic=" << dAs_ana
		...
		for (const auto& vnf : vnFields) {
			...
			{ // single step
				...
				double dk11_meshFD =
					(perturbAll(stepMesh) - perturbAll(-stepMesh)) / (2.0 * stepMesh);

				std::cout << "  vn=" << vnf.name << "  var=" << dk11_var
						  << "  meshFD=" << dk11_meshFD << "  formula=" << dk11_formula;
...
TEST(AsymptoticConductivity, CompareWithMinsurf) {
	...
	for (auto f_it = mesh.faces_begin(); f_it != mesh.faces_end() && checked < 5;
		...
		Eigen::Matrix3d sens_x = (gu_x + pw_x).transpose() *
								 (bform_x - H_x * Eigen::Matrix2d::Identity()) * (gu_x + pw_x);

		Eigen::Matrix<double, 2, 3> gu_m = G_m * ue;
		Eigen::Matrix<double, 2, 3> pw_m = fr_m.leftCols<2>().transpose();
		double H_m = bform_m.trace() / 2.0;
		Eigen::Matrix3d sens_m = (gu_m + pw_m).transpose() *
								 (bform_m - H_m * Eigen::Matrix2d::Identity()) * (gu_m + pw_m);
		...
		std::cout << "  As: xtpms=[" << geom.vrings[i0].As << "," << geom.vrings[i1].As << ","
...
TEST(Surgery, DiagnoseRingOrder) {
	...
	for (auto v_it = mesh.vertices_begin(); v_it != mesh.vertices_end() && checked < 5; ++v_it) {
		...
		std::cout << "v" << vh.idx() << " valence=" << mesh.valence(vh) << " voh=[";
		for (int v : ring_voh)
			std::cout << v << ",";
		std::cout << "] face=[";
		for (int v : ring_face)
		...
		auto buildRing = [&](const std::vector<int>& order) {
			...
			for (int vi : order) {
				...
				ring.push_back(center + Eigen::Vector3d(static_cast<double>(wrapped[0]),
		...
		if (checked == 0) {
			...
			for (size_t ri = 0; ri < ring_voh.size(); ++ri) {
				auto diff =
					mesh.point(xtpms::PeriodicTriMesh::VertexHandle(ring_voh[ri])) - mesh.point(vh);
				auto wrapped = mesh.wrapVector(diff);
				Eigen::Vector3d p = center + Eigen::Vector3d(static_cast<double>(wrapped[0]),
															 static_cast<double>(wrapped[1]),
															 static_cast<double>(wrapped[2]));
				auto raw = mesh.point(xtpms::PeriodicTriMesh::VertexHandle(ring_voh[ri]));
				std::cout << "  ring[" << ri << "] v" << ring_voh[ri] << " raw=(" << raw[0] << ","
						  << raw[1] << "," << raw[2] << ") wrapped=(" << p.transpose() << ")"
						  << " dist=" << (p - center).norm() << "\n";
			}
		}
...
TEST(Surgery, GyroidSurgerySmoke) {
	...
	for (int res : {16, 24, 32}) {
		...
		for (auto& vr : g.vrings) {
			maxH = std::max(maxH, std::abs(vr.H));
			sumH += std::abs(vr.H);
		}
		double avgH = sumH / m.n_vertices();
		std::cout << "res=" << res << " nv=" << m.n_vertices() << " maxH=" << maxH
				  << " avgH=" << avgH << "\n";
	}
...
TEST(Surgery, ExportMergedSplitSingularity) {
	namespace fs = std::filesystem;
	const char* envPath = std::getenv("XTPMS_SURGERY_MESH");
	if (envPath == nullptr || envPath[0] == '\0') {
		GTEST_SKIP() << "Set XTPMS_SURGERY_MESH to an OBJ path to run this test";
	}
	const fs::path meshPath(envPath);
	if (!fs::exists(meshPath)) {
		GTEST_SKIP() << "Mesh not found: " << meshPath.string();
	}
	...
	auto fusedGeom = xtpms::computeVertexGeometry(mesh);
	double fusedMax = 0.0, fusedAvg = 0.0;
	for (auto& vr : fusedGeom.vrings) {
		fusedMax = std::max(fusedMax, std::abs(vr.H));
		fusedAvg += std::abs(vr.H);
	}
	fusedAvg /= static_cast<double>(mesh.n_vertices());

	std::ifstream singIn(splitSing);
	ASSERT_TRUE(singIn.good());
	int singLines = 0;
	double singMax = 0.0, singSum = 0.0;
	std::string line;
	while (std::getline(singIn, line)) {
		if (line.empty())
			continue;
		const double v = std::stod(line);
		singMax = std::max(singMax, v);
		singSum += v;
		++singLines;
	}

	xtpms::DefaultTriMesh splitMesh;
	ASSERT_TRUE(OpenMesh::IO::read_mesh(splitMesh, splitObj));
	EXPECT_EQ(singLines, static_cast<int>(splitMesh.n_vertices()))
		<< "singularity txt lines must match split OBJ vertex count";

	std::cout << "merged fused: nv=" << mesh.n_vertices() << " bnd=" << countBoundaryEdges(mesh)
			  << " fusedMaxH=" << fusedMax << " fusedAvgH=" << fusedAvg << "\n";
	std::cout << "split export: nv=" << splitMesh.n_vertices() << " singLines=" << singLines
			  << " singMax=" << singMax
			  << " singAvg=" << (singLines > 0 ? singSum / singLines : 0.0) << "\n";
	std::cout << "saved: " << splitObj << "\n       " << splitSing << "\n";
}

TEST(Surgery, DiagnoseSingularityAtBoundary) {
	namespace fs = std::filesystem;
	const char* envPath = std::getenv("XTPMS_SURGERY_MESH");
	if (envPath == nullptr || envPath[0] == '\0') {
		GTEST_SKIP() << "Set XTPMS_SURGERY_MESH to an OBJ path to run this test";
	...
	const double faceTol = 1e-3;

	auto onDomainFace = [&](const Vec3d& p) {
		for (int i = 0; i < 3; ++i) {
			const double pi = static_cast<double>(p[i]);
			if (std::abs(pi) < faceTol || std::abs(pi - L[i]) < faceTol)
				return true;
		}
		return false;
	};

	auto singMeasure = xtpms::computeSingularityMeasure(mesh, 2);
	auto geom = xtpms::computeVertexGeometry(mesh);

	auto buildRingWrap = [&](xtpms::PeriodicTriMesh::VertexHandle vh) {
		Eigen::Vector3d center(static_cast<double>(mesh.point(vh)[0]),
							   static_cast<double>(mesh.point(vh)[1]),
							   static_cast<double>(mesh.point(vh)[2]));
		std::vector<Eigen::Vector3d> ring;
		if (mesh.is_boundary(vh)) {
			for (auto voh = mesh.cvoh_iter(vh); voh.is_valid(); ++voh) {
				Vec3d ev =
					mesh.wrapVector(mesh.point(mesh.to_vertex_handle(*voh)) - mesh.point(vh));
			...
			auto he = he_start;
			do {
				Vec3d ev = mesh.wrapVector(mesh.point(mesh.to_vertex_handle(he)) - mesh.point(vh));
				ring.push_back(center + Eigen::Vector3d(ev[0], ev[1], ev[2]));
				he = mesh.opposite_halfedge_handle(mesh.prev_halfedge_handle(he));
			} while (he != he_start && ring.size() < 30);
		}
	...
	entries.reserve(mesh.n_vertices());
	for (auto v_it = mesh.vertices_begin(); v_it != mesh.vertices_end(); ++v_it) {
		const int vi = (*v_it).idx();
		const auto& vr = geom.vrings[static_cast<std::size_t>(vi)];
		const double disc = std::abs(vr.H * vr.H - vr.K);
		const double sqrtDisc = std::sqrt(disc);
		const double singWrap = std::max(std::abs(vr.H + sqrtDisc), std::abs(vr.H - sqrtDisc));
		const auto crWrap = buildRingWrap(*v_it);
		const double discW = std::abs(crWrap.H * crWrap.H - crWrap.K);
		const double sqrtDiscW = std::sqrt(discW);
		const double singFromWrapRing =
			std::max(std::abs(crWrap.H + sqrtDiscW), std::abs(crWrap.H - sqrtDiscW));
		entries.push_back({vi,
						   singMeasure[static_cast<std::size_t>(vi)],
						   vr.H,
						   vr.K,
						   singFromWrapRing,
						   onDomainFace(mesh.point(*v_it)),
						   static_cast<int>(mesh.valence(*v_it)),
						   mesh.point(*v_it)});
		(void)singWrap;
	}
	...
	int highOnFace = 0, highInterior = 0;
	const double highThr = 100.0;
	double sumAsFace = 0, sumAsInt = 0;
	int cntFace = 0, cntInt = 0;
	for (const auto& e : entries) {
		const auto& vr = geom.vrings[static_cast<std::size_t>(e.vi)];
		if (e.onFace) {
			sumAsFace += vr.As;
			++cntFace;
		} else {
	...
	std::cout << "fused mesh nv=" << mesh.n_vertices() << " highSing(>" << highThr
			  << ") onFace=" << highOnFace << " interior=" << highInterior << "\n";
	std::cout << "avg As onFace=" << (cntFace ? sumAsFace / cntFace : 0)
			  << " interior=" << (cntInt ? sumAsInt / cntInt : 0) << " (nv face=" << cntFace
			  << " int=" << cntInt << ")\n";
	std::cout << "top10 fused singularity (makePeriod ring via computeVertexGeometry):\n";
	for (int i = 0; i < 10 && i < static_cast<int>(entries.size()); ++i) {
		const auto& e = entries[static_cast<std::size_t>(i)];
		std::cout << "  #" << i << " v" << e.vi << " sing=" << e.sing
				  << " singWrapRing=" << e.singWrap << " H=" << e.H << " K=" << e.K
				  << " val=" << e.valence << " onFace=" << e.onFace << " pos=(" << e.pos[0] << ","
				  << e.pos[1] << "," << e.pos[2] << ")\n";
	}
	...
	std::vector<double> splitSingVals;
	{
		std::ifstream in(splitSing);
		std::string line;
		while (std::getline(in, line)) {
			if (!line.empty())
				splitSingVals.push_back(std::stod(line));
		}
	}
	ASSERT_EQ(splitSingVals.size(), splitMesh.n_vertices());

	int maxSplitVi = 0;
	for (int i = 1; i < static_cast<int>(splitSingVals.size()); ++i) {
		if (splitSingVals[static_cast<std::size_t>(i)] >
			splitSingVals[static_cast<std::size_t>(maxSplitVi)])
			maxSplitVi = i;
	}
	const auto maxPos = splitMesh.point(splitMesh.vertex_handle(maxSplitVi));
	std::cout << "split max singularity v" << maxSplitVi
			  << " val=" << splitSingVals[static_cast<std::size_t>(maxSplitVi)] << " pos=("
			  << maxPos[0] << "," << maxPos[1] << "," << maxPos[2] << ")\n";
	bool splitOnFace = onDomainFace(maxPos);
	std::cout << "split max on domain face=" << splitOnFace << "\n";

	// dump neighbor geometry for top fused outlier
	if (!entries.empty()) {
		const int topVi = entries[0].vi;
		const auto topVh = mesh.vertex_handle(topVi);
		std::cout << "top v" << topVi << " neighbors:\n";
		for (auto voh = mesh.cvoh_iter(topVh); voh.is_valid(); ++voh) {
			const int ni = mesh.to_vertex_handle(*voh).idx();
			const auto np = mesh.point(mesh.vertex_handle(ni));
			const auto raw = np - mesh.point(topVh);
			const auto mp = xtpms::PeriodicTriMesh::Vec3d(static_cast<double>(raw[0]),
														  static_cast<double>(raw[1]),
														  static_cast<double>(raw[2]));
			(void)mp;
			std::cout << "  n" << ni << " pos=(" << np[0] << "," << np[1] << "," << np[2]
					  << ") rawDiff=(" << raw[0] << "," << raw[1] << "," << raw[2] << ")\n";
		}
	}
}

TEST(Surgery, ExternalMeshFromEnv) {
	namespace fs = std::filesystem;
	const char* envPath = std::getenv("XTPMS_SURGERY_MESH");
	if (envPath == nullptr || envPath[0] == '\0') {
		GTEST_SKIP() << "Set XTPMS_SURGERY_MESH to an OBJ path to run this test";
	...
	const Vec3d hp(1.0, 1.0, 1.0);
	auto mesh = loadPeriodicMeshLikeCli(src, hp);
	const int bndBefore = countBoundaryEdges(mesh);
	const int nfBefore = static_cast<int>(mesh.n_faces());
	std::cout << "external mesh: nv=" << mesh.n_vertices() << " nf=" << nfBefore
			  << " bnd=" << bndBefore << "\n";

	// Singularity uses intrinsic Delaunay flips on a connectivity copy (see
	// computeSingularityMeasure)
	auto sing = xtpms::computeSingularityMeasure(mesh, 2);
	double maxSing = 0.0, avgSing = 0.0;
	for (double s : sing) {
		maxSing = std::max(maxSing, s);
		avgSing += s;
	}
	if (!sing.empty())
		avgSing /= static_cast<double>(sing.size());
	...
	const std::string preFairPath = (outDir / "after_fill_pre_fair.obj").string();
	const std::string filledPath = (outDir / "after_fill.obj").string();
	ASSERT_TRUE(mesh.saveUnitCell(beforePath));

	xtpms::SurgeryOptions opts;
	opts.singularityTol = 25.0;
	opts.surgeryType = 2;
	opts.preFillDumpPath = holesPath;			// excised mesh with holes, before CGAL fill
	opts.postFillPreFairDumpPath = preFairPath; // filled, before bilaplacian
	bool performed = mesh.surgery(opts);
	if (!performed) {
		std::cout << "default tol=25 did not trigger surgery, retry tol=5\n";
		opts.singularityTol = 5.0;
		performed = mesh.surgery(opts);
	}
...
TEST(AsymptoticConductivity, ObjectiveK00) {
	Eigen::Matrix3d kA;
	kA << 0.4, 0.01, 0.0, 0.01, 0.3, 0.0, 0.0, 0.0, 0.2;
	...
	// 曲率分布统计
	{
		...
		if (flatLen < 0) {
			...
			for (auto e = mesh.edges_begin(); e != mesh.edges_end(); ++e) {
				...
				auto ev =
					mesh.point(mesh.to_vertex_handle(he)) - mesh.point(mesh.from_vertex_handle(he));
				totalEdgeLen += ev.norm();
				++edgeCnt;
			}
		...
		std::cout << "actual edge len: min=" << minEdge << " max=" << maxEdge
				  << " avg=" << sumEdge / edgeCnt2 << "\n";
	}
	...
	// Post MCF smooth：纯 MCF 跑若干步看效果
	{
		...
		for (int mcfIter = 0; mcfIter < 20; ++mcfIter) {
			...
			for (auto v_it = mesh.vertices_begin(); v_it != mesh.vertices_end(); ++v_it) {
				int vi = (*v_it).idx();
				Eigen::Vector3d lx(
					geomMcf.vrings[vi].Lx[0], geomMcf.vrings[vi].Lx[1], geomMcf.vrings[vi].Lx[2]);
				maxLx = std::max(maxLx, lx.norm());
				auto p = mesh.point(*v_it);
				mesh.set_point(
					*v_it,
					Vec3d(p[0] + static_cast<xtpms::DefaultTriMesh::Scalar>(mcfStep * lx[0]),
			...
			if (mcfIter % 5 == 0 || mcfIter == 19)
				std::cout << "  mcf iter=" << mcfIter << " APAC=" << apacMcf << " maxLx=" << maxLx
						  << "\n";
		}
	...
	auto src = makeTPMSMC(
		...
		[Lx, Ly, Lz](double x, double y, double z) {
			...
			double perturb =
				...
				0.4 * std::sin(px) * std::cos(2 * pz) + 0.3 * std::sin(2 * px) * std::cos(2 * py) +
...
TEST(AsymptoticConductivity, TailorAPAC_Rod3) {
	...
	for (int pre = 0; pre < 5; ++pre) {
		...
		for (auto v_it = mesh.vertices_begin(); v_it != mesh.vertices_end(); ++v_it) {
			...
			double step = 0.05;
			mesh.set_point(*v_it,
						   Vec3d(p[0] + static_cast<xtpms::DefaultTriMesh::Scalar>(step * lx[0]),
		...
		std::cout << "  pre " << pre << ": nv=" << mesh.n_vertices() << " nf=" << mesh.n_faces()
				  << " maxLx=" << maxLx << " bnd=" << countBoundaryEdges(mesh) << "\n";
	}
