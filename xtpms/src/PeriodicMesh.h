...
// Periodic mesh: cross-period edge detection, period-aware neighborhood access; periodizeFrom uses
...
// Vertex/edge periodic boundary flag bits: bit0=x_min, bit1=x_max, bit2=y_min, bit3=y_max,
// bit4=z_min, bit5=z_max
struct BoundaryFlag {
...
// periodizeFrom: translate mesh, periodic replication + AABB narrow-band voxels + signed distance +
// periodic symmetry + Marching Cubes to generate periodic mesh.
struct PeriodizeOptions {
	double bboxPaddingWorld{0.0};
	double bboxPaddingFraction{0.0};
	// When >0: attempt to collapse boundary edges shorter than this length (requires OpenMesh
	// is_collapse_ok). 0 means no collapse.
	double collapseShortBoundaryEdgesBelow{0.0};

	/// AABB padding of the fundamental domain \([0,2\cdot\text{halfPeriod}]\)^3 (written to
	/// resolvedBBox*). Number of voxel cells per axis \(N\) (grid points \(N+1\)), domain is
	/// \([0,L_x]\times[0,L_y]\times[0,L_z]\).
	int mcCellsPerAxis{32};
	/// Number of dilation layers in the 26-neighborhood for surface-intersecting voxels (fills
	/// narrow band, asymmetric corners).
...
struct SurgeryOptions {
	// Aligned with the mainstream minsurf settings (surgery-tol=25 surgery-type=2, used by 8/14
	// scripts)
	...
	double singularityRatio{0.0}; // Adaptive ratio (0 = disabled, use absolute threshold only)
								  // max(tol, ratio * avgH)
	double maxCutAreaFraction{
		0.3}; // Maximum fraction of cut area; surgery is aborted if exceeded (safety valve)
	...
	std::string preFillDumpPath{}; // After excision, before CGAL fill
	std::string
		postFillPreFairDumpPath{}; // After hole fill (faces added), before bilaplacian fairing
};
...
public:
	...
	// Periodize: copy -> (optional) collapse short boundary edges -> translate AABB min to origin
	// -> 3x3x3 periodic replication -> AABB tree -> narrow-band voxels -> grid-point SDF (sign from
	// closest face normal) -> periodic grid-point group weighted average -> Marching Cubes writes
	// back to this mesh. If &naiveMesh == this, the mesh is copied first before processing.
	...
	// Periodic boundary merging: edge matching + split alignment + vertex welding, turning a
	...
	// Starting from the seed vertex of each connected component, BFS along 3 axes accumulating
	// coordinate differences after periodic wrapping. If the accumulated distance along an axis >=
	// one full period, the component spans that axis. A component is kept as long as it spans at
	// least one axis; otherwise it is removed. Returns the number of deleted faces.
	int removeNonPeriodicIslands();

	// Keep only the largest connected component by face count; return the number of removed
	// components. Used to clean up residual islands after surgery, preventing multi-dimensional
	// null space in the FEM Laplacian.
	int keepLargestComponent();

	// Truncate cross-period edges at periodic boundaries; the mesh is no longer periodically closed
	// after truncation. splitEdges=true: Phase 1 splits edges at boundaries + Phase 2
	// dupPeriodFaces splitEdges=false: Phase 2 dupPeriodFaces only (avoids bumps introduced by
	// splitting)
	void splitUnitCell(bool splitEdges = true);

	// Save unit cell. When split=true, the mesh is copied and splitUnitCell is applied before
	...
	// Writes OBJ and a txt with one singularity value per vertex line (OBJ vertex order).
	bool saveSplitUnitCellWithSingularity(const std::string& objPath,
